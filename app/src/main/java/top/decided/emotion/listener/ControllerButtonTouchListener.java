...
public class ControllerButtonTouchListener implements View.OnTouchListener {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_DOWN){
            ConnectionService.getInstance().getController().setPressed(Integer.parseInt((String) v.getTag()), true);
            MainActivity.pressedVibration(Config.isButtonVibration(), Config.getVibrator());
        }else if(event.getAction() == MotionEvent.ACTION_UP) {
            ConnectionService.getInstance().getController().setPressed(Integer.parseInt((String) v.getTag()), false);
        }
        return false;
    }
